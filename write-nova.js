...
// ── nova/page.tsx — server wrapper ────────────────────────────────────────────
const novaPage = `import { getScaffolds } from "@/lib/actions/scaffold-actions";
...
export default async function NovaInspecaoPage() {
  ...
  return <NovaInspecaoForm scaffolds={scaffolds} />;
}
`;

// ── nova/nova-inspecao-form.tsx — client component ────────────────────────────
const novaForm = `"use client";
...
type ScaffoldOption = { id: string; code: string; location: string; area: string };

function statusToPrisma(status: string): "CL_OK" | "CL_FAIL" | "CL_WARN" | "CL_NA" {
  if (status === "conforme") return "CL_OK";
  if (status === "nao_conforme") return "CL_FAIL";
  return "CL_NA";
}

export function NovaInspecaoForm({ scaffolds }: { scaffolds: ScaffoldOption[] }) {
  const router = useRouter();
  const params = useSearchParams();

  const [selectedScaffoldId, setSelectedScaffoldId] = useState(params.get("scaffold_id") ?? "");
  ...
  const canSubmit = isComplete && !!selectedScaffoldId && inspectorName.trim().length > 0 && !submitting;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createInspection({
        ...
        result: autoResult as "aprovado" | "aprovado_com_ressalvas" | "reprovado",
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-10">
      ...
      <div className="pb-4 border-b-2 border-border">
        <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1">AndCheck EHS · NR-18 / NR-35 / ABNT NBR 6494</p>
        <h1 className="text-[18px] font-bold text-foreground tracking-tight uppercase">Nova Inspeção</h1>
      ...
      <div className="bg-card border border-border shadow-sm p-5 space-y-4">
        <h3 className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border pb-2">Informações Gerais</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <Select value={selectedScaffoldId} onValueChange={setSelectedScaffoldId}>
              ...
              <SelectContent>
                {scaffolds.map((s) => (
                  <SelectItem key={s.id} value={s.id}>{s.code} — {s.location}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            ...
            <Input placeholder="Nome completo" value={inspectorName} onChange={(e) => setInspectorName(e.target.value)} className="h-8 text-[11px] rounded-none" />
          </div>
        </div>
        <div className="space-y-1.5">
          <Label className="text-[10px] uppercase tracking-wider font-bold">Validade da liberação</Label>
          <Select value={validityDays} onValueChange={setValidityDays}>
            <SelectTrigger className="w-40 h-8 text-[11px] rounded-none"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className={"border p-4 flex items-center gap-4 " + (autoResult === "aprovado" ? "bg-emerald-50 border-emerald-200" : autoResult === "reprovado" ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200")}>
          ...
          {autoResult !== "reprovado" && (
            <div className="ml-auto text-right">
              ...
              <p className="text-[13px] font-bold font-mono text-foreground">{format(addDays(new Date(), Number(validityDays)), "dd/MM/yyyy")}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="bg-card border border-border shadow-sm p-5 space-y-3">
        <h3 className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border pb-2">Observações Gerais</h3>
        <Textarea placeholder="Registre observações gerais sobre a inspeção..." value={observations} onChange={(e) => setObservations(e.target.value)} className="text-[11px] rounded-none min-h-[80px]" />
      </div>

      <div className="flex flex-col sm:flex-row gap-3 justify-end pt-2 border-t border-border">
        <Link href="/inspecoes" className="inline-flex items-center justify-center h-8 px-5 text-[10px] font-bold uppercase tracking-widest border border-border hover:bg-muted/50 transition-colors">
          Cancelar
        </Link>
        <button
          type="button"
        ...
        >
          ...
          {submitting ? "Registrando..." : isComplete ? "Registrar Inspeção" : "Preencha todos os itens"}
        </button>
      </div>
    </div>
  );
...
fs.writeFileSync(
  "src/app/(dashboard)/inspecoes/nova/page.tsx",
  novaPage,
  "utf8",
);
fs.writeFileSync(
  "src/app/(dashboard)/inspecoes/nova/nova-inspecao-form.tsx",
  novaForm,
  "utf8",
);
console.log(
  "nova/page.tsx ok",
  fs.statSync("src/app/(dashboard)/inspecoes/nova/page.tsx").size,
);
console.log(
  "nova-inspecao-form.tsx ok",
  fs.statSync("src/app/(dashboard)/inspecoes/nova/nova-inspecao-form.tsx").size,
);
